/*
  # Database Migration for Reinee E-commerce Platform

  1. Schema Updates
    - Add `category` and `shopee_link` columns to products table
    - Convert `price` column from numeric to text type with proper data migration
    - Create `product_colors` table for normalized color storage
    - Add `created_at` column to hero table

  2. Security Setup
    - Enable RLS on all tables
    - Create public read policies (no authentication required for browsing)
    - Create admin-only write policies for data management

  3. Data Migration
    - Insert all collections (Heels, Mary Jane, Sandals, Platform)
    - Insert hero section data
    - Insert all 22 products with proper collection references
    - Insert all product colors in normalized format

  4. Important Notes
    - Safely converts existing numeric price data to text format
    - Maintains data integrity during type conversion
    - Uses conflict resolution to prevent duplicate data
*/
